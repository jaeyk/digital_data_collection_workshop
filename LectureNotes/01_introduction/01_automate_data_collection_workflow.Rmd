...
hits <- map_dbl(url_list, find_hit)
...
- Let me reinforce the idea why `purrr:map()` is awesome. 
...
6. In case, if you want to reduce the listed items you scraped ... 

- Automatically scrapped thousands newspaper articles, parsed them, and now you want to put all of these elements on the list into a single dataframe. 

- The key is using `reduce()` function.
...
jsonl_to_df_all <- function(dir_path){
      ...
      filename <- list.files(dir_path, # This is also a pretty neat technique for parsing PDF files stored in a particular directory.
                pattern = '^x',
                full.names = TRUE)
