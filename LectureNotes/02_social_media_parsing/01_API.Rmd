...
3. Process: 
    ...
    - Parsing the target data your laptop/database 
        - Laptop (sample-parse): Downsamle the large target data and parase it onr your laptop. This is just one option to [deal with big data in R](https://rviews.rstudio.com/2019/07/17/3-big-data-strategies-for-r/). It's a simple strategy as it doesn't require to store your target data in your own database. 

    - Database (push-parse): Push the large target datat to a database, then explore, select, and filter it. If you were interestd in using this option, then check out my [SQL for R Users](https://github.com/dlab-berkeley/sql-for-r-users) workshop.
...
- But what exactly is this target data?
    ...
    - When you access social media data through API, you deal with either XML or mostly JSON (major formats for storing and transporting data; they are light and flexible).
